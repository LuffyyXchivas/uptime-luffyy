...
console.log(`Tüm Komutlar Yüklendi Bot Çevrimiçi`);
files.forEach(f => {
...
client.on("ready", () => {
  client.channels.cache.get("938873924962353234").join();
})
...
link.forEach(l => {
ping(l.link)
console.log(`Ziyaret : ${l.link}`)
})
})
}, 100000)

require('http').createServer((req, res) => res.end('Web Sitesi Hazır')).listen(3000)

client.on('ready', async () => {
  client.user.setPresence({ activity: { name: "!help | Dm Den Kullan", type: "PLAYING" }, status: "online" })
